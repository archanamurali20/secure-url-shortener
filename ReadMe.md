...
https://secure-url-shortener-qyvm.onrender.com/docs

### Note
 - The first request may take up to 50 seconds while the free instance wakes from idle. 
 - SQLite lives on ephemeral disk in the render environment, so the database resets on every redeploy or restart.
### Suggestion
Start from the auth/register endpoint to create a new user and authorize with the correct credentials to test the other endpoints.
...
| POST    | `/links`         | Creates a code for the given url         | 200 with assigned code <br>401 unauthenticated <br>422 on validation error <br> 429 when rate limited                                       |
| GET     | `/links/{code}`  | Gets the link for the code               | 200 with the corresponding link<br>404 on code not found <br> 429 when rate limited                                |
| POST    | `/auth/register` | Creates a user account                   | 200 with name and email,<br>409 on duplicate email, <br>422 on validation failure <br> 429 when rate limited  |
| POST    | `/auth/login`    | Exchanges credentials for a JWT          | 200 with access token, <br>401 on bad credentials, <br> 429 when rate limited          |
| GET     | `/all-links`     | Lists the authenticated user's links     | 200 with list of codes and URLs,<br>401 without a valid token <br> 429 when rate limited                          |
| DELETE  | `/links/{code}`  | Deletes a link you own                   | 204 on success, <br>401 unauthenticated, <br>403 if not yours, <br>404 if no such code <br> 429 when rate limited |
...
5. Password Storage
    - Argon2 for hashing - OWASP's current recommendation, and resistant to GPU-based cracking
    ...
    - The password hash is excluded from API responses by the response model.
6. Authentication
    ...
    - Dummy hash verified on the unknown-email path so response timing doesn't leak registration status.
...
7. Authorization
    ...
    - `DELETE /links/{code}` checks ownership against the authenticated user's id before acting
...
8. Rate limiting
    ...
    - Link creation and code deletion capped to limit abuse
    - The remaining endpoints are also limited except `/health`
